...
for path in kernel/arch/x86_64/boot.S kernel/arch/x86_64/kernel.c kernel/arch/x86_64/kernel.h kernel/graphics/framebuffer.c kernel/graphics/framebuffer.h kernel/boot/multiboot2.c kernel/boot/multiboot2.h kernel/process/elf.c kernel/process/elf.h kernel/process/process.c kernel/process/process.h kernel/fs/rootfs.c kernel/fs/rootfs.h kernel/syscall.c kernel/syscall.h kernel/linker.ld kernel/Makefile userspace/Makefile userspace/linker.ld userspace/build-rootfs.py userspace/libc/mantle.h userspace/libc/start.S userspace/libc/syscalls.S userspace/libc/mantle-string.c build/make-image.sh boot/grub/grub.cfg tests/verify-image.sh tests/verify-userspace.sh tests/test-multiboot2.c tests/test-ring3-frame.c tests/qemu-boot.sh tests/qemu-graphics.sh tests/verify-kernel-source.sh sources.lock; do
