...
MB2="$ROOT/kernel/boot/multiboot2.c"
...
grep -q 'MANTLE_MB2_MAGIC_OK' "$MAIN"
grep -q 'MANTLE_MB2_MODULE_FOUND' "$MAIN"
grep -q 'MANTLE_ROOTFS_NOT_FOUND' "$MAIN"
grep -q 'ROOTFS_START' "$MAIN"
grep -q 'MANTLE_RING3_PREPARE' "$MAIN"
grep -q 'MANTLE_RING3_IRET' "$MAIN"
grep -q 'MANTLE_RING3_ENTRY_OK' "$MAIN"
grep -q 'MANTLE_USER_STACK_OK' "$MAIN"
grep -q '0x13ull << 48u' "$MAIN"
grep -q '"/bin/hello"' "$MAIN"
grep -q 'cmdline hex' "$MAIN"
grep -q 'mantle_mb2_parse_module' "$MB2"
