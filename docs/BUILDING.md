# Construire MantleOS

## Linux ou CI

La chaîne active nécessite GCC, binutils, GNU Make, les modules GRUB EFI et
PC, xorriso et mtools. Le test de boot nécessite QEMU et OVMF. Sur Ubuntu :

```sh
sudo apt-get update
sudo apt-get install -y build-essential gcc binutils make grub-efi-amd64-bin grub-pc-bin grub-common xorriso mtools qemu-system-x86 ovmf python3
MANTLE_PROFILE=personal sh ./build.sh
sh ./tests/verify-image.sh
sh ./tests/qemu-boot.sh
```

Le résultat est `build/out/mantleos-amd64.iso`. Le script écrit également sa
somme SHA-256, `mantleos-build-info.txt`, `build.log` et, après le test QEMU,
`qemu-serial.log`.

Le build compile le noyau freestanding et les programmes userspace natifs
depuis le dépôt. Il ne télécharge ni Linux, ni BusyBox, ni musl, ni une archive
de distribution. Le rootfs est un module MantleOS vérifié, transmis au noyau
par Multiboot2.

## Windows

Le dépôt ne compile pas directement un noyau ELF depuis PowerShell. Utiliser
Docker Desktop Linux :

```powershell
.\build.ps1 -Profile personal
```

Le conteneur est un environnement d’outillage de build uniquement ; il ne
définit pas l’architecture de MantleOS.

## Compilation directe du noyau

Dans un environnement POSIX équipé de GCC et binutils :

```sh
make -C kernel clean all
```

Le fichier produit est `build/work/kernel/mantle-kernel.elf`. La cible ISO
exécute aussi `grub-file --is-x86-multiboot2` avant `grub-mkrescue`.

## CI

Le workflow `.github/workflows/mantleos-build.yml` installe les outils de
construction, vérifie la frontière d’architecture, compile le userspace et le
rootfs MantleOS, produit l’ISO, vérifie son hash, démarre l’image en UEFI QEMU
et attend les marqueurs kernel, framebuffer, rootfs, ELF, ring 3, init et shell
console. Une erreur de boot fait échouer le job. L’ISO et les diagnostics sont
publiés comme artefacts uniquement après réussite.

Le premier test framebuffer graphique, sans prétendre fournir encore une
session Wayland, peut être lancé séparément :

```sh
sh ./tests/qemu-graphics.sh
```

Il attend `MANTLE_GRAPHICS_OK`, écrit uniquement après que le kernel a reçu un
framebuffer Multiboot2 et dessiné le splash MantleOS. `MANTLE_DESKTOP_OK` n’est
pas émis : Wayland et le bureau graphique ne sont pas encore embarqués.
