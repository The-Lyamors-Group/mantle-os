...
int main(void){
    ...
    char *up[]={(char*)"/bin/ip",(char*)"link",(char*)"set",(char*)"dev",iface,(char*)"up",NULL};run(up);
    if(!strcmp(mode,"static") && address[0]){
        char *addr[]={(char*)"/bin/ip",(char*)"addr",(char*)"replace",address,(char*)"dev",iface,NULL};run(addr);
        if(gateway[0]){char *route[]={(char*)"/bin/ip",(char*)"route",(char*)"replace",(char*)"default",(char*)"via",gateway,NULL};run(route);}
    ...
    for(int i=0;i<100;i++){if(access("/run/mantle/network.ready",F_OK)==0){serial_line("MANTLE_NETWORK_OK");break;}usleep(100000);}
